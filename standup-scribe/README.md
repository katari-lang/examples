...
- **Ask controls as data**, checked where they are built (`check_controls`) and fitted honestly
  (`string.fit` against `slack.limits()`) instead of discovering Slack's caps from a rejected payload.
...
- **One-shot model use**: a single `ai.infer` drafts the digest — no chat loop, no tools, one
...
- **A desk and its region**: a sequential handler as the collection actor, fibers as sources, each
  fiber supervising itself rather than the region inventing an answer for it:
...
- **The window lives in the store, not in the fiber.** `standup/prompts/<date>` holds
  ...
  re-opens the same window, a schedule fiber run again after a crash resumes it before arming its
...
- **One sequential desk** (`main`'s `var window` handler) is the collection actor: `idle` or
  ...
  `crypto.pseudonym` (a keyed HMAC under the bot token) before they are stored or shown to the
...
- **Each fiber supervises itself, and the region policy is what is left.** `slack.provider` connects
  nothing — it serves the two tokens, resolved per call, and the Socket Mode connection belongs to the
  one call that needs events. So a call the runtime interrupted is answered *inside* the fiber, by
  `use supervise.forever()` plus a panic converter: the same body runs again after a backoff, resolves
  the tokens and opens its own connection. `region.crashed` therefore has nothing left to do, and
  `region.failed` is the one real decision — an uncaught throw is a dead token, a Slack refusal to open
  the socket, or configuration drift, and it stops the run. Nothing above a fiber is ever rebuilt,
  which is what keeps the desk's collected replies across the re-run.
...
- **A runtime restart keeps the open window *and* the replies already collected in it.** Every Slack
  ...
  the one call that was in flight. That is normally the channel watch, which its own supervisor runs
  again — a fresh Socket Mode connection, collecting again. The desk is
  ...
  durable timer, not an external call), and if it *was* in a Slack call, the attempt that replaces it
...
- The tick is **at-least-once**. A redelivered occurrence does *not* repost the prompt, and does not
  ...
  nothing committed to say so. The re-run fiber finds no marker and stays quiet (that day is
...
- An open review does **not** survive a runtime restart: the ask rides one external call, so a restart
  ...
  day's digest is not posted. The schedule fiber runs again, finds today's window already closed,
...
- Only a failure that will not heal stops the run: an auth error, configuration drift, or a Slack
  ...
  somebody notices. A **panic** takes the other road and is run again by the fiber's own supervisor,
  unbounded, with the backoff ceiling as the only brake (`supervise.forever()`'s defaults: one second,
  doubling, capped at fifteen minutes) — so a genuine defect that panics on every attempt settles into
  one attempt a quarter-hour rather than stopping. That is why the boot validates the hour, the window
  and the zone: `time.cron` panics on a zone it does not know, and the supervisor would run the fiber
  straight back into it. `katari status <run>` and its events are where such a loop is visible, not the
  channel.
